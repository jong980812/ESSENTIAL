...
def train_class_batch(model, samples, target, criterion,
                      actionlist,actiondict,actiontoken,mask,args,device):
    ...
    outputs_action, actionFeature = model(samples, actionlist,actiondict,actiontoken)
    if featnorm:
        outputs_action = outputs_action / outputs_action.norm(dim=-1, keepdim=True)
        actionFeature = actionFeature / actionFeature.norm(dim=-1, keepdim=True)
        action_logits = outputs_action @ actionFeature.t() / 0.07# B, Classes(200)
    else:
        action_logits = outputs_action @ actionFeature.t() / 0.07

    if mask is not None:
        not_mask = np.setdiff1d(np.arange(args.nb_classes), mask)
        not_mask = torch.tensor(not_mask, dtype=torch.int64).to(device)
        action_logits = action_logits.index_fill(dim=1, index=not_mask, value=float('-inf'))
    loss = criterion(action_logits, action_targets)
    return loss, outputs_action,action_logits
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast():
                loss, output,logit = train_class_batch(
                model, samples, targets, criterion,actionlist,actiondict,actiontoken,mask,args,device)
        ...
        top1_acc,_ = accuracy(logit, targets ,topk=(1, 5))
...
def evaluate(model: torch.nn.Module,  data_loader, 
            device, task_id=-1, class_mask=None,class_list=None, args=None,header=None):
    ...
    with torch.no_grad():
        for batch in metric_logger.log_every(data_loader, 10, header):
            ...
            with torch.cuda.amp.autocast():
                outputs_action, actionFeature = model(videos, actionlist,actiondict,actiontoken)
                if featnorm:
                    outputs_action = outputs_action / outputs_action.norm(dim=-1, keepdim=True)
                    actionFeature = actionFeature / actionFeature.norm(dim=-1, keepdim=True)
                    action_logits = outputs_action @ actionFeature.t() / 0.07# B, Classes(200)
            ...
            acc1, acc5 = accuracy(action_logits, action_targets, topk=(1, 5))
...
def evaluate_till_now(model: torch.nn.Module, data_loader, 
                    device, task_id=-1, class_mask=None,class_list=None, acc_matrix=None, args=None,test_mode=False):
    ...
    for i in range(task_id+1):
        if test_mode:
            ...
            test_stats = evaluate(model=model, data_loader=data_loader[i]['test'], 
                                device=device, task_id=i, class_mask=class_mask,class_list=class_list, args=args,header=header)
        else:
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, 
                    ...
                    class_mask=None, args = None,loss_scaler=None,class_lists=None):

    cil_actionlist,cil_actiondict,cil_actiontoken, all_class_list=class_lists
    ...
    for task_id in range(args.num_tasks):
        # class_list = cil_actionlist[task_id],cil_actiondict[task_id],cil_actiontoken[task_id]
        # # ! Debug
        # if task_id < 5:
        #     continue
       ...
       # lr scehdule
        ...
        for epoch in range(args.epochs): 
            ...
            train_stats = train_one_epoch(model=model, criterion=criterion, 
                                        ...
                                        update_freq=args.update_freq, header= header,loss_scaler=loss_scaler,class_list=class_list
                                        )
        ...
        val_stats = evaluate_till_now(model=model, data_loader=data_loader, device=device, 
                                    task_id=task_id, class_mask=class_mask,class_list = class_list, acc_matrix=acc_matrix, args=args,test_mode=False)
