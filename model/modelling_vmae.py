...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 attn_head_dim=None, dim_mlp=192):
        super().__init__()
    ...
    def forward(self, x):
        if self.gamma_1 is None:
            ...
            # ln2 = self.norm2(x)
            # x = x + self.drop_path(self.mlp(ln2)) + 0.5*self.adapter(ln2) 
            ln2 = self.norm2(x)
            x = x + self.drop_path(self.mlp(ln2))
        else:
    ...
    def __init__(self, 
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, dim_mlp=dim_mlp)
