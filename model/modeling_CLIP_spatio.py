...
class ResidualAttentionBlock_time(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return xs
class ResidualAttentionBlock_spatial(nn.Module):
    def __init__(self, space_mode:str,d_model: int,drop_path=0.,dim_mlp=192,skip_connect=True):
        super().__init__()
        self.space_mode = space_mode
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.space_down = nn.Linear(d_model,dim_mlp)
        self.space_up = nn.Linear(dim_mlp,d_model)
        self.space_act = nn.GELU()
        self.skip_connect = skip_connect
        self.initial_adapter()
    def initial_adapter(self):
        print('Transformer for cls is initialized')
        for n, m in self.space_up.named_modules():
            for n2, m2 in m.named_modules():
                if isinstance(m2, nn.Linear):
                    nn.init.constant_(m2.weight, 0)
                    nn.init.constant_(m2.bias, 0)
    def forward(self, x: torch.Tensor):
        #입력 cls_token B,T,D
        xs = self.space_down(x)
        xs = self.space_act(xs)
        xs = self.space_up(xs)
        if self.skip_connect:
            x = x + xs
        else:
...
class CLIP_spatio(nn.Module):
    ...
    def __init__(self, input_resolution: int, num_frames: int, patch_size: int, width: int, layers: int, heads: int, drop_path_rate, num_tadapter=1, adapter_scale=0.5, pretrained=None,num_classes=400,init_scale=0.001,spatial_type='avg',dropout_ratio=0.2,dim_mlp=192,args=None):
        ...
        self.temporal_for_cls = nn.Sequential(*[ResidualAttentionBlock_time(self.temp_mode,width, self.ba_heads, None,0., num_tadapter, num_frames, drop_path=drop_path_rate,dim_mlp=dim_mlp) for _ in range(self.ba_layers)])
        self.spatial_for_cls = nn.Sequential(*[ResidualAttentionBlock_spatial(None,width,0.,dim_mlp,True) for _ in range(self.ba_layers) ])
    ...
    def forward(self, x: torch.Tensor, train=False,task_id =-1):
        ...
        x = self.ln_post(x)
        x = x[:, 0]
        x_spatial = self.spatial_for_cls(x)#! bt d
        x_spatial = rearrange(x_spatial, '(b t) d -> b d t',b=B,t=T)
        x = rearrange(x, '(b t) d -> b t d',b=B,t=T)
        x = x + self.temporal_embedding
        x = rearrange(x, 'b t d -> t b d',b=B,t=T)
        x_time = self.temporal_for_cls(x)
        x_time = rearrange(x_time, 't b d -> b d t',b=B,t=T)
        x_final = rearrange(x_time,'b d t -> b t d',b=B,t=T)
