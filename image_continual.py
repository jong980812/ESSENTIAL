...
from image_continual_utils import read_image_file, read_label_file
...
def build_image_dataloader(args):
    dataloader = list()
    class_mask = list() #if args.task_inc or args.train_mask else None

    transform_train = build_image_transform(True, args)
    transform_val = build_image_transform(False, args)

    if args.data_set.startswith('Split-'):
        dataset_train, dataset_val = get_dataset(args.data_set.replace('Split-',''), transform_train, transform_val, args)

        args.nb_classes = len(dataset_val.classes)

        splited_dataset, class_mask = split_single_dataset(dataset_train, dataset_val, args)
    else:
        if args.data_set == '5-datasets':
            dataset_list = ['SVHN', 'MNIST', 'CIFAR10', 'NotMNIST', 'FashionMNIST']
        else:
            dataset_list = args.data_set.split(',')
    ...
    for i in range(args.num_tasks):
        ...
        dataloader.append({'train': data_loader_train, 'val': data_loader_val,'test':data_loader_val})

    return dataloader, class_mask,n_vids_per_task,None
...
def build_image_transform(is_train, args):
